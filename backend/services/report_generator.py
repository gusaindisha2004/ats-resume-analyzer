...
def generate_html_reports(analysis_data: Dict) -> Dict[str, str]:
    ...
    overall_score = analysis_data.get('ats_score', 0)
    ...
    svd_raw = analysis_data.get('skill_validation') or {}
    ...
    jd_raw = analysis_data.get('jd_match')
    ...
    # Order matters — the PDF exporter concatenates these in dict order.
    reports = {
        'summary':        env.get_template('summary.html').render(**context),
        'skills':         env.get_template('action_items.html').render(**context),
        'action_items':   env.get_template('jd_comparison.html').render(**context),
    }
    if jd_raw:
        reports['jd_match'] = env.get_template('quick_actions.html').render(**context)
    return reports